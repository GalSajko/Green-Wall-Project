...
def all_legs_positions(joints_values, fk_type, legs = spider.LEGS_IDS):
...
@numba.jit(nopython = True, cache = False)
def leg_forward_kinematics(joints_values_in_leg, legs_dimensions = spider.LEGS_DIMENSIONS):
    """Calculate forward kinematics for spiders leg, using transformation matrices.  

    Args:
        joints_values_in_leg (list): 1x3 array of joints values in leg, in radians.
...
def leg_base_to_third_joint_forward_kinematics(joints_values_in_leg):
...
@numba.jit(nopython = True, cache = False)
def spider_base_to_leg_tip_forward_kinematics(leg_id, joints_values_in_leg, angle_between_legs = spider.ANGLE_BETWEEN_LEGS, radius = spider.BODY_RADIUS, legs_dimensions = spider.LEGS_DIMENSIONS):
...
def platform_forward_kinematics(legs_ids, legs_positions_in_global, legs_poses_in_spider):
    """Calculate forward kinematics of platform. Use only those legs, that are in contact with pins.

    Args:
        legs_ids (list): Ids of used legs.
        legs_positions_in_global (list): Global positions of used legs.
        legs_poses_in_spider (list):4x4 transformation matrices, representing a legs poses in spider's origin.
...
def get_spider_pose(legs_ids, legs_positions_in_global, joints_values):
    ...
    Args:
        ...
        legs_positions_in_global (list): nx3 array of x, y, z positions of used legs in global origin, where n should be the same as length of legs_ids.
...
def get_goal_pin_in_local(leg_id, attached_legs_ids, legs_positions_in_global, joints_values, rpy):
    """Calculate goal pin position in leg-local origin.

    Args:
        leg_id (int): Leg id.
        attached_legs_ids (list): List of attached legs.
        legs_positions_in_global (list): 5x3 array of global legs positions.
        joints_values (list): 5x3 array of joints values.
...
@numba.jit(nopython = True, cache = False)
def leg_inverse_kinematics(leg_position_in_local, legs_dimensions = spider.LEGS_DIMENSIONS):
    """Calculate inverse kinematics for leg, using geometry.

    Args:
        leg_position_in_local (list): Desired position of end effector in leg-base origin.
...
#region jacobians bv  
@numba.jit(nopython = True, cache = False)
def leg_jacobi(joints_values_in_leg, legs_dimensions = spider.LEGS_DIMENSIONS):
...
@numba.jit(nopython = True, cache = False)
def spider_base_to_leg_tip_jacobi(leg_id, joints_values_in_leg, angle = spider.ANGLE_BETWEEN_LEGS, legs_dimensions = spider.LEGS_DIMENSIONS):
...
@numba.jit(nopython = True, cache = False)
def get_joints_velocities(joints_values, legs_velocities, number_of_legs = spider.NUMBER_OF_LEGS, number_of_motors_in_leg = spider.NUMBER_OF_MOTORS_IN_LEG):
    ...
    Args:
        ...
        legs_velocities (list): 5x3 array of x, y, z velocities of all legs.
...
def get_xd_xdd_from_offsets(force_mode_legs_ids, offsets_in_spider, velocities_in_spider, spider_to_leg_transformations = spider.T_ANCHORS):
    ...
    Args:
        force_mode_legs_ids (list): List of legs' ids, that are used in force controll.
        offsets_in_spider (list): Calculated offsets from force-position P controller.
        velocities_in_spider (list): Calculated velocities from force-position P controller.
